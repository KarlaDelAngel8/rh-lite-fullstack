<!-- Este módulo sirve para: Vista del catálogo de Sucursales. Permite listar, crear, editar y desactivar sucursales con búsqueda y paginación. -->
...
const areaName = computed(() => route.query.areaName || 'Area sin nombre');
const areaId = computed(() => route.query.areaId);
...
function toRow(item) {
  return {
    id: item.id ?? item.branch_id ?? item.uuid,
...
async function fetchBranches() {
  ...
  errorMessage.value = '';
  try {
    const params = areaId.value ? { area_id: areaId.value } : {};
    const { data } = await BranchesApi.all(params);
    rows.value = normalizeRows(data).map(toRow);
  } catch (error) {
    errorMessage.value = error?.response?.data?.message || error.message || 'No se pudo cargar el catalogo de sucursales.';
  } finally {
    loading.value = false;
  }
}
...
async function submitForm() {
  ...
  if (areaId.value) {
    payload.area_id = areaId.value;
  }

  try {
    if (isEditing.value && selectedId.value != null) {
      await BranchesApi.update(selectedId.value, payload);
    } else {
    ...
    errorMessage.value = error?.response?.data?.message || error.message || 'No se pudo guardar la sucursal.';
  } finally {
    saving.value = false;
  }
}
...
async function toggleStatus(item) {
  ...
  try {
    await BranchesApi.changeStatus(item.id, payload);
    await fetchBranches();
  } catch (error) {
    errorMessage.value = error?.response?.data?.message || error.message || 'No se pudo actualizar el estado de la sucursal.';
  }
}
  ...
  <section class="catalog-page">
    <header class="catalog-header">
      <div>
        ...
        <h1>Sucursales</h1>
        <p>Inicio / Áreas / {{ areaName }} / Sucursales</p>
      </div>
    </header>

    <section class="table-card">
      <div class="table-toolbar">
        <button class="btn-primary" @click="openCreateModal">+ Agregar sucursal</button>
        ...
        <table>
          ...
          <tbody>
            ...
            <tr v-if="visibleRows.length === 0">
              <td colspan="3">No hay sucursales para mostrar.</td>
            </tr>
          </tbody>
        </table>
      </div>
    ...
    <div v-if="showModal" class="modal-overlay" @click="closeModal">
      <div class="modal" @click.stop>
        <div class="modal-header">
          <h2>{{ isEditing ? 'Editar Sucursal' : 'Agregar Sucursal' }}</h2>
        ...
        <form class="modal-body" @submit.prevent="submitForm">
          <label>
            Nombre
            <input v-model="form.name" type="text" placeholder="Ingresa el nombre de la sucursal" />
          </label>
