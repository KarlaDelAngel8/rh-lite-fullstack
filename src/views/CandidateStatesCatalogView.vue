<!-- Este módulo sirve para: Vista del catálogo de Estados de Candidatos. Permite listar, crear, editar y desactivar los estados del proceso de reclutamiento con búsqueda y paginación. -->
<!-- Elaborado por: Karla Vanessa Del Angel Santiago -->

<script setup>
import { computed, onMounted, reactive, ref } from 'vue';
import CandidateStatesApi from '@/api/CandidateStates';
...
const filteredRows = computed(() => {
  ...
  return rows.value.filter((item) => String(item.name || '').toLowerCase().includes(term));
});
...
function toRow(item) {
  return {
    id: item.id ?? item.candidate_state_id ?? item.uuid,
...
async function fetchStates() {
  ...
  errorMessage.value = '';
  try {
    const { data } = await CandidateStatesApi.all();
    rows.value = normalizeRows(data).map(toRow);
    currentPage.value = 1;
  } catch (error) {
    errorMessage.value = error?.response?.data?.message || error.message || 'No se pudo cargar el catálogo de estados de candidatos.';
  } finally {
    loading.value = false;
  }
}
...
async function submitForm() {
  if (!form.name.trim()) {
    errorMessage.value = 'El nombre del estado es obligatorio.';
    return;
  }
  ...
  try {
    if (isEditing.value && selectedId.value != null) {
      await CandidateStatesApi.update(selectedId.value, payload);
    } else {
      await CandidateStatesApi.create(payload);
    }
    closeModal();
    await fetchStates();
  } catch (error) {
    errorMessage.value = error?.response?.data?.message || error.message || 'No se pudo guardar el estado.';
  } finally {
    saving.value = false;
  }
}
...
async function toggleStatus(item) {
  ...
  try {
    await CandidateStatesApi.changeStatus(item.id, { status: isActiveStatus(item.status) ? 0 : 1 });
    await fetchStates();
  } catch (error) {
...
<template>
  <section class="catalog-page">
    <header class="catalog-header">
      <h1>Estados de Candidatos</h1>
      <p>Inicio / Estados de Candidatos</p>
    </header>

    <section class="table-card">
      <div class="toolbar-top">
        <button class="btn-primary" @click="openCreateModal">+ Agregar Estado de candidatos</button>
      </div>
        ...
        <table>
          ...
          <tbody>
            ...
            <tr v-if="visibleRows.length === 0">
              <td colspan="3">No hay estados de candidatos para mostrar.</td>
            </tr>
          </tbody>
        </table>
      </div>

      <p v-else>Cargando estados de candidatos...</p>
    ...
    <div v-if="showModal" class="modal-overlay" @click="closeModal">
      <div class="modal" @click.stop>
        <div class="modal-header">
          <h2>{{ isEditing ? 'Editar Estado de candidato' : 'Agregar Estado de candidato' }}</h2>
        ...
        <form class="modal-body" @submit.prevent="submitForm">
          <label>
            Nombre
            <input v-model="form.name" type="text" placeholder="Ingresa el nombre del estado" />
          </label>
