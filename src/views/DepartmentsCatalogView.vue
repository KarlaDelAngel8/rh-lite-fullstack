<!-- Este módulo sirve para: Vista del catálogo de Departamentos. Permite listar, crear, editar y desactivar departamentos con búsqueda y paginación. -->
...
function toRow(item) {
  return {
    id: item.id ?? item.department_id ?? item.uuid,
...
async function fetchDepartments() {
  ...
  errorMessage.value = '';
  try {
    const { data } = await DepartmentsApi.all();
    rows.value = normalizeRows(data).map(toRow);
  } catch (error) {
    errorMessage.value = error?.response?.data?.message || error.message || 'No se pudo cargar el catalogo de departamentos.';
  } finally {
    loading.value = false;
  }
}
...
async function submitForm() {
  if (!form.name.trim()) {
    errorMessage.value = 'El nombre del departamento es obligatorio.';
    return;
  }
  ...
  try {
    if (isEditing.value && selectedId.value != null) {
      await DepartmentsApi.update(selectedId.value, payload);
    } else {
    ...
    errorMessage.value = error?.response?.data?.message || error.message || 'No se pudo guardar el departamento.';
  } finally {
    saving.value = false;
  }
}
...
async function toggleStatus(item) {
  ...
  try {
    await DepartmentsApi.changeStatus(item.id, payload);
    await fetchDepartments();
  } catch (error) {
    errorMessage.value = error?.response?.data?.message || error.message || 'No se pudo actualizar el estado del departamento.';
  }
}
...
<template>
  <section class="catalog-page">
    <header class="catalog-header">
      <h1>Departamentos</h1>
      <p>Inicio / Departamentos</p>
    </header>

    <section class="table-card">
      <div class="table-toolbar">
        <button class="btn-primary" @click="openCreateModal">+ Agregar departamento</button>
      ...
      <p v-else>Cargando departamentos...</p>
    </section>

    <div v-if="showModal" class="modal-overlay" @click="closeModal">
      <div class="modal" @click.stop>
        <div class="modal-header">
          <h2>{{ isEditing ? 'Editar Departamento' : 'Agregar Departamento' }}</h2>
        ...
        <form class="modal-body" @submit.prevent="submitForm">
          <label>
            Nombre
            <input v-model="form.name" type="text" placeholder="Ingresa el nombre del departamento" />
          </label>

          <label>
            Descripción
            <textarea v-model="form.description" rows="3" placeholder="Ingresa una descripción" />
          </label>
