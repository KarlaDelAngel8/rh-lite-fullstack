<!-- Este módulo sirve para: Vista del catálogo de Puestos. Permite listar, crear, editar y desactivar puestos de trabajo con búsqueda y paginación. -->
...
function toRow(item) {
  return {
    id: item.id ?? item.position_id ?? item.uuid,
...
async function fetchPositions() {
  ...
  errorMessage.value = '';
  try {
    const { data } = await PositionsApi.all();
    rows.value = normalizeRows(data).map(toRow);
  } catch (error) {
    errorMessage.value = error?.response?.data?.message || error.message || 'No se pudo cargar el catálogo de puestos.';
  } finally {
    loading.value = false;
  }
}
...
async function submitForm() {
  if (!form.name.trim()) {
    errorMessage.value = 'El nombre del puesto es obligatorio.';
    return;
  }
  ...
  try {
    if (isEditing.value && selectedId.value != null) {
      await PositionsApi.update(selectedId.value, payload);
    } else {
    ...
    errorMessage.value = error?.response?.data?.message || error.message || 'No se pudo guardar el puesto.';
  } finally {
    saving.value = false;
  }
}
...
async function toggleStatus(item) {
  ...
  try {
    await PositionsApi.changeStatus(item.id, payload);
    await fetchPositions();
  } catch (error) {
    errorMessage.value = error?.response?.data?.message || error.message || 'No se pudo actualizar el estado del puesto.';
  }
}
  ...
  <section class="catalog-page">
    ...
    <section class="table-card">
      <div class="table-toolbar">
        <button class="btn-primary" @click="openCreateModal">+ Agregar puesto</button>
        ...
        <table>
          ...
          <tbody>
            ...
            <tr v-if="visibleRows.length === 0">
              <td colspan="4">No hay puestos para mostrar.</td>
            </tr>
          </tbody>
        </table>
      </div>

      <p v-else>Cargando puestos...</p>
    </section>

    <div v-if="showModal" class="modal-overlay" @click="closeModal">
      <div class="modal" @click.stop>
        <div class="modal-header">
          <h2>{{ isEditing ? 'Editar Puesto' : 'Agregar Puesto' }}</h2>
        ...
        <form class="modal-body" @submit.prevent="submitForm">
          <label>
            Nombre
            <input v-model="form.name" type="text" placeholder="Ingresa el nombre del puesto" />
          </label>
